...
export default withRouter(class Articles_commands extends Component {
    ...
    getList = (page) => {
        ...
        let listUrl = apiUrl + 'admin/article/comment-list?pageLimit=' + pageLimit + '&page=' + page;
    ...
    updateAccount = (action, id) => {
        ...
        if (updateStatus) {
            axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
            axios.post(apiUrl + 'admin/article/update-comments-status', {
                action: action,
                _id: id,
            }).then((result) => {
    ...
    render() {
        return (
            <div>
                <Head>
                    <meta charSet="utf-8" />
                    <title>{site_name} - Articles  List </title>
                </Head>
                <div className="page-wrapper" id="page-wrapper">
                            ...
                            <div className="box bread-box is-shadowless has-background-white">
                                <nav className="breadcrumb" aria-label="breadcrumbs">
                                    <ul>
                                        ...
                                        <li className="is-active"><a href="#">Articles </a></li>
                                        <li className="is-active"><a href="#">Comments  List</a></li>
                                    </ul>
                                    <Link href="/manage_articles_commands" as="manage_articles_commands">
                                        <a className="ad-new" >Add New Comment</a>
                                    </Link>
                                </nav>
                    ...
                    <div className="box is-shadowless has-background-white" >
                        ...
                        <div className="table-responsive dash-table-res">
                            <div className="level">
                                <h2 className="title is-size-5 has-text-grey-dark is-uppercase is-marginless">Comments   List </h2>
                                <div>
                                </div>
                            ...
                            <table className="table is-bordered is-striped is-narrow is-hoverable is-fullwidth is-size-6">
                                <thead>
                                    <tr className="bg-light">
                                        <th>#</th >
                                        <th >Article Title</th>
                                        <th onClick={this.handleSort.bind(this, 'commentDesc', this.state.sortOrder)} >Comments <i className={`fa ${(this.state.sortKey == "commentDesc") ? this.state.sortClass : "fa-sort"}`}></i></th>
                                        <th onClick={this.handleSort.bind(this, 'commentAdded', this.state.sortOrder)} >Created On <i className={`fa ${(this.state.sortKey == "commentAdded") ? this.state.sortClass : "fa-sort"}`}></i></th>
                                        <th >Status</th>
                ...
                (props.loading) ?
                    ...
                    : (props.arrlist.length > 0) ?
                        props.arrlist.map(function (dataRow, i) {
                            ...
                            var enabledBtn = (dataRow.commentDisabled == 0) ? true : false;
                            var disbaledBtn = (dataRow.commentDisabled == 1) ? true : false;

                            return <tr key={sNo + i}>
                                <td>{sNo + i}</td>
                                <td>{dataRow.articleId ? dataRow.articleId.title : ''}
                                </td>
                                <td>{dataRow.commentDesc ? dataRow.commentDesc.substring(0, 10) : ''}
                                </td>
                                <td>{dataRow.commentAdded.slice(0, 10)}</td>
                                <td>
                                    {
                                        dataRow.commentDisabled === 0 ?
                                ...
                                <td>
                                    <div className="buttons">
                                        ...
                                        <Link href={`/manage_articles_commands?id=${dataRow._id}`} as={`/update-comment/${dataRow._id}`}>
