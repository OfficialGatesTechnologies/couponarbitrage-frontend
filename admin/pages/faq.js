...
export default withRouter(class Faq extends Component {
    ...
    getList = (page) => {
        ...
        let listUrl = apiUrl + 'admin/masterdata/faq-list?pageLimit=' + pageLimit + '&page=' + page;
    ...
    updateFaqs = (action, id) => {
        ...
        if (updateStatus) {
            axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
            axios.post(apiUrl + 'admin/masterdata/update-faq-status', {
                action: action,
                _id: id,
            }).then((result) => {
                ...
                <div className="page-wrapper" id="page-wrapper">
                            ...
                            <div className="box bread-box is-shadowless has-background-white">
                                <nav className="breadcrumb" aria-label="breadcrumbs">
                                    <ul>
                                        ...
                                        <li className="is-active"><a href="#">Faqs   List</a></li>
                                    </ul>
                                    <Link href="/manage_faq">
                                        <a className="ad-new" >Add New Faq </a>
                                    </Link>
                                </nav>
                    ...
                    <div className="box is-shadowless has-background-white" >
                        ...
                        <div className="table-responsive dash-table-res">
                            <div className="level">
                                <h2 className="title is-size-5 has-text-grey-dark is-uppercase is-marginless">Faqs    List </h2>
                                <div>
                            ...
                            <table className="table is-bordered is-striped is-narrow is-hoverable is-fullwidth is-size-6">
                                <thead>
                                    <tr className="bg-light">
                                        ...
                                        <th onClick={this.handleSort.bind(this, 'faqQuestion', this.state.sortOrder)} >Question   <i className={`fa ${(this.state.sortKey == "faqQuestion") ? this.state.sortClass : "fa-sort"}`}></i></th>
                                        <th  >Category </th>
                                        <th style={{ width: '200px' }}>Action</th>
                                    </tr>
                                </thead>
                                <TableListContent updateFaqs={this.updateFaqs} pageLimit={this.state.pageLimit} activePage={this.state.activePage} arrlist={this.state.arrList} loading={this.state.loading} />
                    ...
                    : (props.arrlist.length > 0) ?
                        props.arrlist.map(function (dataRow, i) {
                            ...
                            return <tr>
                                ...
                                <td>{dataRow.faqQuestion}</td>

                                <td>{dataRow.faqCategory}</td>


                                <td>
                                    <div className="buttons">

                                        <Link href={`/manage_faq?id=${dataRow._id}`} as={`/manage_faq/${dataRow._id}`}>
                                            <a data-tip="Edit" className="button is-info is-small tooltip" data-tooltip="Edit">  <span className="icon has-text-white">
                                                <i className="fas fa-pencil-alt"></i>
                                            </span></a>
                                        </Link>

                                        <button data-tip="Delete" onClick={props.updateFaqs.bind(this, 'delete', dataRow._id)} className="button is-danger is-small tooltip" data-tooltip="Delete">
