...
export default withRouter(class manage_cashback_payout extends Component {
    ...
    getRow = (editId) => {
        ...
        axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
        axios.get(apiUrl + 'admin/cashback-payouts/payout-row?_id=' + editId)
            .then(res => {
    ...
    handleSubmit = (e) => {
        ...
        this.formValidation(fieldNeedToValidate, (isValid) => {
            if (isValid) {
                axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
                axios.post(apiUrl + 'admin/cashback-payouts/update-payout', payoutData).then((result) => {
                    let sucMsg = result.data.msg;
                    toastr.success(sucMsg, '');
                    if (payoutData.cb_type == 3) {
                        Router.push(`/revenue_cashback_payouts`);
                    } else {
                        Router.push(`/cashback_bonus_payouts`);
                    }
                            ...
                            <div className="box bread-box is-shadowless has-background-white">
                                <nav className="breadcrumb" aria-label="breadcrumbs">
                                    <ul>
                                        ...
                                        <li>
                                            {
                                                payoutData.cb_type == 3 ?
                                                    <Link href="/revenue_cashback_payouts" prefetch>
                                                        <a href="#"> Revenue Share Cashback Payouts  </a>
                                                    </Link> :
                                                    payoutData.cb_type == 2 ?
                                                        <Link href="/cashback_bonus_payouts" prefetch>
                                                            <a href="#"> Cashback & Bonuses Payouts   </a>
                                                        </Link> :
                                                        <Link href="/affiliate_payouts" prefetch>
                                                            <a href="#"> Affiliate Payouts    </a>
                                                        </Link>
                                            }
                        ...
                        <div className="columns">
                            <div className="column is-3">
                                <div className="control">
                                    ...
                                    <input className={"input " + (_.get(error, 'value') ? ' is-danger' : '')} type="text" name="value" placeholder="value" value={payoutData.value} onChange={this.handleInputChange} onKeyUp={this.onTextFieldBlur} onBlur={this.onTextFieldBlur} />
