...
export default withRouter(class Sbobet_cashback extends Component {
    ...
    getList = (page) => {
        ...
        let listUrl = apiUrl + 'admin/turnover-cashback/sbobet-cashbacks?pageLimit=' + pageLimit + '&page=' + page;
    ...
    uploadCashbacks = () => {
        ...
        this.formValidation(fieldNeedToValidate, (isValid) => {
            if (isValid) {
                ...
                axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
                axios.post(apiUrl + 'admin/turnover-cashback/upload-sbobet-data', data).then((result) => {
    ...
    handleUpdateUser = () => {
        const { user_id, sbobetId } = this.state;
        let fieldNeedToValidate = ['user_id'];
        this.formValidation(fieldNeedToValidate, (isValid) => {
            if (isValid) {
                axios.post(apiUrl + 'admin/turnover-cashback/update-sbobet-user', { user_id: user_id, sbobetId: sbobetId }).then((result) => {
                    let sucMsg = result.data.msg;
                    this.setState({ user_id: '', sbobetId: '', selectedOption: { value: '', label: '' }, visibleModal: false });
                    if (result.data.success) {
                        toastr.success(sucMsg, '');
                        setTimeout(() => { this.getList(1); }, 300);
                    } else toastr.error(sucMsg, 'Error!');
    ...
    render() {
        ...
        return (
            <div>
                <Head>
                    <meta charSet="utf-8" />
                    <title>{site_name}  -  SBObet Cashback s  </title>
                </Head>
                            ...
                            <div className="box bread-box is-shadowless has-background-white">
                                <nav className="breadcrumb" aria-label="breadcrumbs">
                                    <ul>
                                        ...
                                        <li className="is-active"><a href="#">SBObet Cashback  Manager</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                                                ...
                                                <div className="control">
                                                    ...
                                                    <div className="select is-fullwidth">
                                                        <select name="searchBy" onChange={this.handleInputChange}>
                                                            ...
                                                            <option value="sbobetId">User Id</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <table className="table is-bordered is-striped is-narrow is-hoverable is-fullwidth is-size-6">
                                <thead>
                                    <tr className="bg-light">
                                        ...
                                        <th onClick={this.handleSort.bind(this, 'sbobetId', this.state.sortOrder)} >MoneyBooker User Id   <i className={`fa ${(this.state.sortKey == "sbobetId") ? this.state.sortClass : "fa-sort"}`}></i></th>
                                        <th>User Name</th>
                                        <th>E-mail </th>
                ...
                <TagModel selectedOption={this.state.selectedOption} sbobetId={this.state.sbobetId} handleUpdateUser={this.handleUpdateUser} searchUsers={this.searchUsers} handleSelectChange={this.handleSelectChange} visibleModal={this.state.visibleModal} closeTagModal={this.closeTagModal} errors={this.state.error} />
            </div>
        )
    }
                    ...
                    : (props.arrlist.length > 0) ?
                        props.arrlist.map(function (dataRow, i) {
                            ...
                            return <tr>
                                ...
                                <td>{
                                    dataRow.user_id ?
                                        <Link href={`/sbobet_cashback_details?id=${dataRow.sbobetId}`} as={`/sbobet_cashback_details/${dataRow.sbobetId}`}>
                                            <a data-tip="View" className="button is-success is-small tooltip" data-tooltip="View"> {dataRow.user_id.username}</a>
                                        </Link> :
                                        <button onClick={props.openModel.bind(this, dataRow.sbobetId)} className="button is-danger is-small">Assign User</button>
                                }
                                </td>
                                <td>{dataRow.user_id ?
    ...
    return (
        <Modal width="500" visible={props.visibleModal} effect="fadeInUp">
            <header className="modal-card-head">
                <p className="modal-card-title">Assign User {props.sbobetId}</p>
            </header>
