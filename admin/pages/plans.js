...
export default withRouter(class Plans extends Component {
    ...
    getList = (page) => {
        ...
        let listUrl = apiUrl + 'admin/bookmaker/plan-list?pageLimit=' + pageLimit + '&page=' + page;
    ...
    updatePlanStatus = (action, id) => {
        ...
        if (updateStatus) {
            axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtAdminToken');
            axios.post(apiUrl + 'admin/bookmaker/update-plan-status', {
                action: action,
                _id: id,
            }).then((result) => {
                        ...
                        <div className="column">
                            <div className="box bread-box is-shadowless has-background-white">
                                <nav className="breadcrumb" aria-label="breadcrumbs">
                                    ...
                                    <Link href="/manage_plans">
                                        <a className="ad-new" >Add New Plan</a>
                                    </Link>
                                </nav>
                    ...
                    : (props.arrlist.length > 0) ?
                        props.arrlist.map(function (dataRow, i) {
                            return <tr>
                                ...
                                <td>{dataRow.plan_duration} {dataRow.plan_duration_type}</td>
     
                                <td>
                                    <div className="buttons">
                                        
                                        <Link href={`/manage_plans?id=${dataRow._id}`} as={`/update_plan/${dataRow._id}`}>
